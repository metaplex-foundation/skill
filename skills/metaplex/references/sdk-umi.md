...
- Bubblegum writes: spread `getAssetWithProof(...)` and pass leaf-canonical metadata (`currentMetadata: assetWithProof.currentMetadata`, or `asCurrentMetadataV2(assetWithProof)`). Do not pass display `metadata` into write args when royalties are inherited. Core collection updates also need `authority: collectionAuthority` and `coreCollection`.
