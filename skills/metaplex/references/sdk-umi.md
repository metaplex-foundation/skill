...
- Bubblegum writes: spread `getAssetWithProof(...)` and pass `currentMetadata: assetWithProof.currentMetadata`. Do not pass display `metadata` into write args when royalties are inherited. Core collection updates also need `authority: collectionAuthority` and `coreCollection`. Raw DAS `_raw` / `inherited` live on `rpcAsset`.
